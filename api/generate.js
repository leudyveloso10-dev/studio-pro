export default async function handler(req, res) {
  ...
  try {
    ...
    if (images && images.length > 0) {
      ...
      parts.push({ text: 'Analise as imagens enviadas e use os elementos visuais REAIS (produtos, pessoas, cores, ambiente) para criar os prompts de imagem fielmente.' });
    }
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`,
      { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contents: [{ parts }], generationConfig: { temperature: 0.8, maxOutputTokens: 4000 } }) }
    );
...
function buildSystemPrompt(profile) {
  const profiles = {
    ...
    leudy: { name:"Leudy Veloso", desc:"Marketing e Inovação com IA — tecnologia, gastronomia, lifestyle, ensaios pessoais.", tom:"leve, reflexivo, dinâmico e moderno", cores:"bordô #8b0000, dourado #c9a84c, preto #1a1a1a", hashtags:"#leudyveloso #marketingdigital #inteligenciaartificial #gastronomia #lifestyle #inovacao" }
  };
...
{"versoes":[{"legenda":"legenda completa com emojis e CTA (min 120 palavras)","imgEn":"detailed English image prompt","imgPt":"prompt detalhado em português","hashtags":["#tag1","#tag2","#tag3"],"roteiro":"roteiro 30s Reels com cenas numeradas"},{"legenda":"...","imgEn":"...","imgPt":"...","hashtags":[],"roteiro":"..."},{"legenda":"...","imgEn":"...","imgPt":"...","hashtags":[],"roteiro":"..."}]}`;
}
